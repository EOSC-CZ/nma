...
import { Container, Grid, Button, Icon, TransitionablePortal } from "semantic-ui-react";
import { i18next } from "@translations/i18next";
import { SearchAppFacets, SearchAppResultsPane, SearchConfigurationContext } from "@js/invenio_search_ui/components";
import { ResultOptions } from "./ResultOptions";
import { ShouldActiveFiltersRender, ActiveFiltersCountFloatingLabel } from "@js/oarepo_ui/search";
    ...
    <Grid columns={columnsAmount} relaxed className="search-app rel-mt-2" padded>
      ...
      <Grid.Column {...resultsPaneLayout}>
        <Grid>
          ...
          <ShouldActiveFiltersRender>
            <Grid.Row only="computer tablet">{facetsAvailable && <ActiveFilters />}</Grid.Row>
          </ShouldActiveFiltersRender>
          <Grid.Row verticalAlign="middle" className="result-options pb-0">
            <ResultOptionsWithState />
          </Grid.Row>
          <Grid.Row verticalAlign="middle">
            <Grid.Column as="section" aria-label={i18next.t("Search results")} width={16}>
              <SearchAppResultsPane layoutOptions={config.layoutOptions} appName={appName} buildUID={buildUID} />
            </Grid.Column>
          </Grid.Row>
        </Grid>
      </Grid.Column>
      {hasButtonSidebar && (
        <Grid.Column mobile={16} tablet={16} computer={4} largeScreen={4} widescreen={4}>
          <Overridable id={buildUID("SearchApp.buttonSidebarContainer", "", appName)}></Overridable>
        </Grid.Column>
      )}
    </Grid>
  );
...
export const SearchAppLayout = ({ config, hasButtonSidebar }) => {
  ...
  const resultsPaneLayout = facetsAvailable ? resultsPaneLayoutFacets : resultsPaneLayoutNoFacets;

  const resultSortLayout = facetsAvailable ? resultsSortLayoutFacets : resultsSortLayoutNoFacets;
