...
class EuropaLanguagesTypeReader(BaseReader):
    def __init__(
        self,
        *,
        source=None,
    ...
    def __iter__(self) -> Iterator[StreamEntry]:
        ...
        for desc in language_descriptions:
            ...
            language_urls.append(about)

        unprocessed_languages = set(language_urls)
        for url in language_urls:
            loaded, url = self.load_language(url)
            if True:
                # with Pool(30) as pool:
                #     for loaded, url in tqdm.tqdm(
                #         pool.imap(self.load_language, language_urls), total=len(language_urls)
                #     ):
                print("Got", url, flush=True)
                if url in unprocessed_languages:
                    unprocessed_languages.remove(url)
                log.warning(
                    "Unprocessed languages left: %s", len(unprocessed_languages)
                )
                if len(unprocessed_languages) < 50:
                    log.warning("Unprocessed languages left: %s", unprocessed_languages)
                if not loaded.filtered:
                    yield loaded

    def load_language(self, about: str) -> tuple[StreamEntry, str]:
        log.warning("Loading language from url %s", about)
        t1 = time.time()
        resp = requests.get(about, timeout=10)
        if resp.status_code != 200:
            log.error("Unable to fetch language %s: %s", about, resp.status_code)
            return (
                StreamEntry(
                    id=None,
                    filtered=True,
        ...
        log.warning("Loaded language from url %s, took %s", about, time.time() - t1)
        root_node = etree.fromstring(resp.content)
        ...
        if not title:
            log.warning("Language %s has no title in selected languages", code)
            return (
                StreamEntry(
                    id=code,
                    filtered=True,
